...
(function ($) {
	...
	function setUp($el, settings) {
		var width = $el.data('width')
			...
			, youtube_data_url = ['https://www.googleapis.com/youtube/v3/videos?id=', id, '&key=', 'AIzaSyBHp-atIjY-m7152vnrELHYor5pfkfYvao', '&part=snippet'].join('')
		...
		$thumb = $el.find('.ytp-thumbnail').on('click', function (e) {
			...
			if (!$el.hasClass(settings.video_loaded_class)) {
				$el.html('<iframe class="vids" src="http://www.youtube.com/embed/' + id + '?showinfo=0&enablejsapi=1&version=3&playerapiid=ytplayer' + youtube_parameters + '&autoplay=1" frameborder="0" allowscriptaccess="always" allowfullscreen></iframe>').addClass(settings.video_loaded_class);
				// execute callback
		...
		if ((!title && display_title) || display_duration) {
			...
			$.getJSON(youtube_data_url, function (data) {
				...
				if (display_duration) {
					$el.find('.video-time').text(parseDuration(item.contentDetails.duration, settings)).show();
				}
			});
		}
	};
	...
	$.fn.lazyYT = function (yt_api_key, newSettings) {
		var defaultSettings = {
			...
			, callback: null, // Advanced settings
